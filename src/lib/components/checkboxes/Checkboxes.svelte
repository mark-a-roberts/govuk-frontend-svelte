<script lang="ts">
    ...
    let {
      id, name, idPrefix, fieldset,errorMessage, hint, items
    }:ICheckboxes = $props;
</script>
...
{#- The first id should not have a number suffix so it's easy to link to from the error summary component -#}
  ...
  {%- if item.conditional.html %} data-aria-controls="{{ conditionalId }}"{/if -%}
  {%- if item.behaviour %} data-behaviour="{{ item.behaviour }}"{/if -%}
  {%- if itemDescribedBy %} aria-describedby="{{ itemDescribedBy }}"{/if -%}
  ...
  <div class="govuk-checkboxes__conditional {%- if not isChecked %} govuk-checkboxes__conditional--hidden{/if}" id="{{ conditionalId }}">
  ...
  {/if}
  <div class="govuk-checkboxes {%- if params.classes %} {{ params.classes }}{/if}"
