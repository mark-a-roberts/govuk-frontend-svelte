	...
	<div class={['govuk-width-container', containerClasses]}>
			...
			<div class="govuk-footer__navigation">
				{#each navigation as nav}
					<div class="govuk-footer__section govuk-grid-column-{nav.width || 'full'}">
						...
						{#if nav.items?.length}
							{@const listClasses = `govuk-footer__list--columns-${nav.columns ?? ''}`}
							<ul class={['govuk-footer__list', listClasses]}>
								{#each nav.items as item}
									{#if item.href && item.text}
										<li class="govuk-footer__list-item">
											<a class="govuk-footer__link" href={item.href} {...item.attributes}>
												{item.text}
											</a>
										</li>
									{/if}
		...
		<div class="govuk-footer__meta">
			<div class="govuk-footer__meta-item govuk-footer__meta-item--grow">
				{#if meta}
					<h2 class="govuk-visually-hidden">{meta.visuallyHiddenTitle || 'Support links'}</h2>
				...
				<span class="govuk-footer__licence-description">
					{#if contentLicence?.html || contentLicence?.text}
						{@render htmlOrText(contentLicence, 10)}
					{:else}
			...
			<div class="govuk-footer__meta-item">
				...
				>
					{#if copyright?.html || copyright?.text}
						{@render htmlOrText(copyright, 10)}
					{:else}
						© Crown copyright
					{/if}
				</a>
