...
>
	<div class={['govuk-header__container', containerClasses]}>
		...
		{#if serviceName || navigation?.length}
			<div class="govuk-header__content">
				...
				{#if navigation?.length}
					<nav
						...
						class={['govuk-header__navigation', navigationClasses]}
					>
						<button
							type="button"
							class="govuk-header__menu-button govuk-js-header-toggle"
							aria-controls="navigation"
							aria-label={menuButtonLabel && menuButtonLabel !== menuButtonText
								? menuButtonLabel
								: null}
							hidden
						>
								...
								{#if item.text | item.html}
									<li
										class={[
											...
											item.active && 'govuk-header__navigation-item--active'
										]}
									>
										{#if item.href}
											<a class="govuk-header__link" href={item.href} {...item.attributes}>
												{@render htmlOrText(item, 18)}
											</a>
										{:else}
											{@render htmlOrText(item, 18)}
										{/if}
