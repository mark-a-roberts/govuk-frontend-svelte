<script lang="ts">
  ...
  let { id, name, classes, attributes, html, text,  idPrefix, items }: IProps = $props();
</script>
...
{@const tabPanelId = item.id ?? idPrefix + "-" + index }
<div class={["govuk-tabs__panel", (index > 1) && "govuk-tabs__panel--hidden"]} id={ tabPanelId }
...
<div {id} class={["govuk-tabs", classes]}
     {...govukAttributes(attributes) } data-module="govuk-tabs">
